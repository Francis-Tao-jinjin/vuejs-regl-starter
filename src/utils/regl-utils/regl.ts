...
export interface REGLLoader {
    ...
    cache:<StateType, PropType>(
        ...
        profile:boolean,
        flags?:{ [symbol:string]:(state:StateType) => boolean|number}) => (state:StateType) => (prop?:PropType, ...rest:any[]) => void;
...
export function createREGLCache(regl: any, profile:boolean) : REGLLoader {
    const definitionCache:any[] = [];
    const valueCache:any[] = [];

    const commandCache:{ [id:string]:CachedREGLCommand[] } = {};
    const commandDefinitionSet:((state: any) => any)[] = [];
    ...
    function preloadShaders (state: any) {
        if (isMobile()) {
            return;
        }
    ...
    function cacheCommand (
        ...
        commandSpec:any,
        noProfile:boolean,
        flags?:{ [symbol:string]:(state: any) => boolean|number }) {
        ...
        function generateCommand (state: any) {
...
export function safeProp<PropType extends {}> (regl:_REGL.Regl) {
    function next<U extends {}, P> (ret:(string|number|symbol)[], prop:P) {
    ...
    return next<PropType, typeof _prop>([], <typeof _prop><any>((_:any, x:any) => x));
}
