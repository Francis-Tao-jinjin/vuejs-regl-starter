...
void main() {
  ...
  outColor = vec4((colorSum / u_kernelWeight).rgb, textureColor.a);

  // outColor = (
  //     texture(u_texture, v_texCoord) +
  //     texture(u_texture, v_texCoord + vec2( onePixel.x, 0.0)) +
  //     texture(u_texture, v_texCoord + vec2(-onePixel.x, 0.0))) / 3.0;

  // outColor = textureColor.rgba;
  // outColor = textureColor * vec4(v_texCoord, 0.0, 1.0);
